...
class Solution {
    ...
    // dp[i][1] = max(dp[i-1][1], dp[i-1][0] - costs[i])
    public int maxProfit(int[] prices) {
        int profit_ith_day_0_in_hand = 0;
        int profit_i_1th_day_0_in_hand = 0;
        int profit_ith_day_1_in_hand = Integer.MIN_VALUE;
        for (int index = 0; index < prices.length; index++) {
            int temp = profit_ith_day_0_in_hand;
            ...
            profit_ith_day_1_in_hand = Math.max(profit_ith_day_1_in_hand, profit_i_1th_day_0_in_hand - prices[index]);
            profit_i_1th_day_0_in_hand = temp;
        }
        return profit_ith_day_0_in_hand;
    }
}
